...
>('get-single/get-thunk', async (id, { rejectWithValue }) => {
	try {
		return await GET(`pets/${id}`);
	} catch (error: any) {
		const errorMessage =
			error?.message || 'Failed to fetch pet information. Please try again.';
...
const FetchSinglePetReducer = createSlice({
	name: 'pet-single/get',
	initialState,
	reducers: {},
...
export default FetchSinglePetReducer.reducer;
